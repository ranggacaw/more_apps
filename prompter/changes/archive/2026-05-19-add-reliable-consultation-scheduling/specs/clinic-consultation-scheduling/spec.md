...
The system SHALL create or reuse a pending booking only when the selected slot is still locked by the requesting patient with a future expiry, and SHALL hand the patient off to the consultation checkout flow after a successful booking confirmation step.
