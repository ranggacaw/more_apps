...
- **AND** the system issues a verification OTP for that patient
- **AND** the system redirects the newly authenticated patient to `/verify-otp`
...
The system SHALL issue a time-bound one-time password for each newly registered patient verification attempt, SHALL deliver that OTP through the clinic's configured WhatsApp flow, and SHALL require a matching, unexpired code submitted through `/verify-otp` before marking the patient account as verified.

#### Scenario: Valid OTP verifies the patient account
- **WHEN** an unverified patient submits the correct OTP through `/verify-otp` before it expires
...
- **WHEN** an unverified patient submits an incorrect or expired OTP through `/verify-otp`
...
The system SHALL authenticate patients through Laravel's session-based login and logout flow, and it SHALL route unverified patient sessions to `/verify-otp` until verification is complete.
...
- **AND** the system redirects the patient to `/verify-otp` instead of verified operational pages
