...
- [ ] 1.2 Create `ClinicQueueEntry` model with fillable, casts (all timestamp fields to datetime), doctor relationship, scope for active entries, and a static method to generate the next daily queue number

## 2. Admin Queue Backend

- [ ] 2.1 Create `AdminQueueController` with:
...
- [ ] 2.2 Add admin routes: `GET /admin/queue`, `GET /admin/queue/api`, `POST /admin/queue`, `PATCH /admin/queue/{entry}/assign`, `PATCH /admin/queue/{entry}/cancel` — all within `auth + verified + role:admin` middleware

## 3. Doctor Queue Backend

- [ ] 3.1 Add methods to `DoctorDashboardController`:
...
- [ ] 3.2 Add doctor routes: `GET /doctor/queue/api`, `POST /doctor/queue/{entry}/start`, `POST /doctor/queue/{entry}/done` — all within `auth + verified + role:doctor` middleware

## 4. Admin Queue Frontend

- [ ] 4.1 Create `Admin/Queue.jsx` Inertia page with:
...
- [ ] 4.2 Modify `Admin/Dashboard.jsx` to add a walk-in queue summary card with waiting/active counts and a link to `/admin/queue`

## 5. Doctor Queue Frontend

- [ ] 5.1 Add a "Current Walk-In Patient" panel to `Doctor/Dashboard.jsx` showing:
...
- [ ] 5.2 Add polling (every 5 seconds) to refresh the doctor's queue patient status from `/doctor/queue/api`

## 6. Admin Dashboard Backend Update

- [ ] 6.1 Update `AdminDashboardController::__invoke()` to include `queue_summary` (counts of waiting, assigned, in_consultation entries) in the Inertia props

## 7. Validation & Testing

- [ ] 7.1 Test: Admin can add, assign, and cancel queue entries
- [ ] 7.2 Test: Doctor can start and complete walk-in consultations
- [ ] 7.3 Test: Admin queue polling returns updated data after doctor marks done
- [ ] 7.4 Test: Queue number increments daily and resets next day
- [ ] 7.5 Test: Only assigned doctor can start/complete their own queue entry
- [ ] 7.6 Test: Cannot assign a non-waiting entry or assign to an inactive doctor

## Post-Implementation

- [ ] Update AGENTS.md in the project root with the new queue routes and the walk-in queue feature description
